...
const instructors_get_id = async (mreq, mres) => {
  ...
  .populate("students",{ name: 1, email: 1,_id: 1,has_whatsapp: 1, mobile: 1})
  .populate("sessions",{ room_id: 1, created_at: 1,ended_at: 1,_id: 1, participants: 1,is_live:1, previously_reached: 1,recently_reached:1})
  .populate("evaluations",{ student: 1, _id: 1,instructor_evaluation: 1,problems: 1})
  .select({password: 0});
  if (res == undefined || res == null) {
    mres.sendStatus(404).json({ message: "No instructor with such id" });
    return;
  }
...
const instructors_put_id_newpass = (mreq,mres)=> {
  ...
  bcrypt.hash(mreq.body.password, 10, function (err, hash) {
    ...
    instructor
      ...
      .catch((err) => {
        mres.sendStatus(400).json({ message: err });
      });
  });
}
...
const instructors_delete_id = (mreq, mres) => {
  Instructor.findByIdAndDelete(mreq.params.id, function (err) {
    if (err) mres.sendStatus(404).json({ message: err });
